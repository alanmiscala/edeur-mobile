...
export function getElapsedMs(deur: Deur): number {
  const total = deur.activities.reduce((sum, a) => {
    if (a.endTime) return sum + a.durationMs;
    return sum + (Date.now() - new Date(a.startTime).getTime());
  }, 0);
  return total;
}
...
export function getActivityTotals(deur: Deur): Record<ActivityType, number> {
  ...
  for (const a of deur.activities) {
    ...
    if (a.activity in totals) {
      totals[a.activity] += dur;
    }
  }
  return totals;
}
...
export function getGrossProductiveTime(deur: Deur): number {
  const totals = getActivityTotals(deur);
  return totals.Operating + totals.Waiting;
}

export function getTotalShiftTime(deur: Deur): number {
  const totals = getActivityTotals(deur);
  return totals.Operating + totals.Waiting + totals.Breakdown;
}
